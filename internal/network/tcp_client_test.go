...
func TestTCPClient_SendAndReceive(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T) *serverHandle {
				return startServer(t, func(conn net.Conn) {
					_, _ = conn.Read(make([]byte, 4))
					time.Sleep(50 * time.Millisecond) // дольше, чем idle timeout клиента
				})
			},
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			client, err := NewTCPClient(slogdiscard.NewDiscardLogger(), opts...)
...
func TestNewTCPClient_NilLogger(t *testing.T) {
	t.Parallel()

	client, err := NewTCPClient(nil, WithClientTCPAddress("127.0.0.1:0"), WithClientTCPMaxMessageSize(8))
	require.Error(t, err)
